...
import 'package:warehouse_web_app/features/label_print_location/models/location_model.dart';
...
import '../models/location_dummy_data.dart';

/// Location page: Location / Description listing table with pagination.
...
class _LocationPageState extends State<LocationPage> {
  ...
  final List<LocationModel> _allRecords = LocationDummyData.records;
  ...
  List<LocationModel> get _pageItems {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          SectionTitle(
            title: 'Location',
            subtitle: '$totalItems locations found',
            trailing: Wrap(
              spacing: AppSizes.spaceSm,
              children: [
                ElevatedButton.icon(
                  onPressed: () => showActionDialog(context, action: 'Add Location'),
                          ...
                          onSelectAll: (selectAll) {
                            setState(() {
                              if (selectAll ?? false) {
                                _selectedLocations
                                  ..clear()
                                  ..addAll(records.map((r) => r.location));
                              } else {
                              ...
                              : records.map((item) {
                                  ...
                                  return DataRow(
                                    ...
                                    onSelectChanged: (selected) {
                                      setState(() {
                                        if (selected ?? false) {
                                          _selectedLocations.add(item.location);
                                        } else {
                                          _selectedLocations
